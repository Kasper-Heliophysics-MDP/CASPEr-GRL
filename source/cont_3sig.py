...
def bin_spectrogram(array):
    """
    Downsamples a 2D array by averaging blocks of (FREQ_BIN_FACTOR, TIME_BIN_FACTOR).
    ...
    print(f"Binning by: ({FREQ_BIN_FACTOR} freqs, {TIME_BIN_FACTOR} time steps)")
    
    # --- 1. Trim Frequency Axis ---
    freq_trim = array.shape[0] % FREQ_BIN_FACTOR
    if freq_trim > 0:
        array = array[:-freq_trim, :]
        
    # --- 2. Trim Time Axis ---
    time_trim = array.shape[1] % TIME_BIN_FACTOR
    if time_trim > 0:
        array = array[:, :-time_trim]
        
    # --- 3. Reshape and Bin ---
    # (freq, time) -> (freq_chunks, FREQ_BIN_FACTOR, time_chunks, TIME_BIN_FACTOR)
    binned_shape = (array.shape[0] // FREQ_BIN_FACTOR, FREQ_BIN_FACTOR, 
                    array.shape[1] // TIME_BIN_FACTOR, TIME_BIN_FACTOR)
...
if __name__ == "__main__":
    ...
    # --- 4. Bin Data ---
    binned_spectrogram = bin_spectrogram(spectrogram_data)
