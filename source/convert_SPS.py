...
def get_args()->Tuple[str, str, bool, bool, bool]:
    source_dir = config.SPS_DUMP_STN

    dest_dir = config.NPY_DIR_STN

    output = config.SHOW_SPS_STN

    make_npy = config.MAKE_NPY_STN

    make_csv = config.MAKE_CSV_STN

    # Check source file
    if not os.path.exists(source_dir):
        raise RuntimeError(f"ERROR: SPS directory not found: {source_dir}")
    if not os.path.isdir(source_dir):
        raise RuntimeError(f"ERROR: Source path is not a dir: {source_dir}")
    ...
    if not os.path.exists(dest_dir):
        try:
            ...
            raise RuntimeError(f"ERROR: Cannot create destination folder '{dest_dir}': {e}")
    elif not os.path.isdir(dest_dir):
        raise RuntimeError(f"ERROR: Destination path is not a directory: {dest_dir}")

    return source_dir, dest_dir, output, make_npy, make_csv
...
def convert_sps_fits(sweep_data: np.ndarray, sps_header: dict, file_name: str, destination_dir: str,
    ...
    """
    if config.COMPRESS_STN:
        sweep_data = resize(sweep_data, (128, 128), anti_aliasing=True)
...
def convert_sps():
    ...
    for file_path in sps_files:
        with open(file_path, "rb") as file:
            ...
            sweep_array = np.array(sweep_data, dtype=np.uint16).T
    ...
    print(f"\n*\t{config.GREEN}Converted all files!{config.RESET}")
