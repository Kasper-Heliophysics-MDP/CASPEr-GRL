...
def get_args() -> Tuple[str, dict[str, Any]]:
    ...
    print(f" \tLogging API Calls: {config.LOG_DBX}")
    print(f" \tSampling Rate: {config.SAMPLE_RATE_DBX}")
    print(f" \tIs a Dry Run (no downloading): {config.DRY_RUN_DBX}")
    print(f" \tCreate output file of added files: {config.OUTPUT_DBX}")
    print(f" \tCreate sub-directories to match Dropbox: {not config.FLAT_DBX}")
    print(f" \tFile types to ignore: {config.EXCLUDE_DBX}")
    print(f" \tFile types to ONLY include: {config.WANT_DBX}")
    print("")
